...
class LinkedDataApiGraph extends PueliaGraph {
	...
	#Antonis botch job
	$escaped_slashes = json_encode($container);
	return str_replace('\/', '/', $escaped_slashes); 
    }
    ...
    function to_simple_xml($pageUri){
        ...
        $version->appendChild($dom->createTextNode('2.2'));
  ...
  function get_all_predicates(){
    ...
    function get_label($uri, $capitalize = false, $use_qnames = FALSE){
        if($label = $this->_config->get_first_literal($uri, RDFS_LABEL)) return $label;
        if($label = $this->_config->get_first_literal($uri, API.'name')) return $label;
        if($label = $this->_config->get_first_literal($uri, API.'label')) return $label;
