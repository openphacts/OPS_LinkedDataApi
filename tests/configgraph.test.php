...
class ConfigGraphTest extends PHPUnit_Framework_TestCase {
    ...
    function test_getFormatters(){
        ...
        'rdfjson' => "http://purl.org/linked-data/api/vocab#RdfJsonFormatter",
        ...
        'xml' => "http://purl.org/linked-data/api/vocab#XmlFormatter",
         ),$this->Graph->getFormatters(),"getFormatters should return all the formatters"
        );
  ...
  function test_getFormatterUriByNameWhenMoreThanOneOfSameName(){
    $fOne = 'http://example.com/formatter/html_one';
    $fTwo = 'http://example.com/formatter/html_two';
    $this->Graph->add_resource_triple($fOne, RDF_TYPE, API.'Formatter');
    $this->Graph->add_literal_triple($fOne, API.'name', 'html');
    $this->Graph->add_resource_triple($fTwo, RDF_TYPE, API.'Formatter');
    $this->Graph->add_literal_triple($fTwo, API.'name', 'html');

    $this->Graph->resetApiAndEndpoint('_:api', '_:endpoint');
    $this->Graph->add_resource_triple('_:endpoint', API.'defaultFormatter', $fTwo);
    $this->Graph->add_resource_triple('_:api', API.'defaultFormatter', $fOne);
    $this->Graph->add_resource_triple('_:endpoint', API.'formatter', $fOne);
    $this->Graph->add_resource_triple('_:endpoint', API.'formatter', $fTwo);
    
    $this->assertEquals($fTwo, $this->Graph->getFormatterUriByName('html'), 'ambiguous name "html" should return the default formatter uri: '. $fTwo);

    $this->Graph->remove_property_values('_:endpoint', API.'defaultFormatter');
    $this->Graph->add_resource_triple('_:endpoint', API.'defaultFormatter', $fOne);
    $this->Graph->resetApiAndEndpoint('_:api', '_:endpoint');
    
    $this->assertEquals($fOne, $this->Graph->getFormatterUriByName('html'), 'ambiguous name "html" should return the default formatter uri:'. $fOne);
